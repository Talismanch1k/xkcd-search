...
func TestUpdate(t *testing.T) {
	...
	require.True(t,
		res1 == http.StatusAccepted && res2 == http.StatusAccepted,
		"wrong statuses from concurrent updates, expect accepted & accepted",
	)
...
func login(t *testing.T) string {
	...
	// unified json replies in api service
	var reply struct {
...
func prepare(t *testing.T) {
	// wait for previous updates to finish
	for {
		s, err := status()
		if err == nil && s == "idle" {
			break
